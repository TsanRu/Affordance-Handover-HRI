...
class SemanticBrainNode:
    ...
    def trigger_callback(self, msg):
        ...
        try:
            ...
            self.target_mode = data.get("mode", "full")
        except json.JSONDecodeError:
            self.target_object = msg.data
            self.target_mode = "full"

        rospy.loginfo(f"Received trigger, target: {self.target_object}, mode: {self.target_mode}")
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            if self.need_process and self.latest_image is not None:
                ...
                self.process(self.latest_image, self.target_object, self.target_mode)
                self.is_processing = False
            rate.sleep()
    ...
    def process(self, img_msg, object_name, mode="full"):
        ...
        try:
            ...
            if mode == "owl_only":
                self.done_pub.publish(json.dumps({
                    ...
                    "owl_score": float(best_pred['score']),
                }))
                return
            ...
            mask_path = os.path.join(self.save_dir, "target_mask.png")
