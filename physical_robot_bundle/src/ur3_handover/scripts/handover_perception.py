...
class HandoverPerceptionNode:
    def __init__(self):
        ...
        self.rgb_depth_sync_slop = float(self._get_param(
            "~rgb_depth_sync_slop", "/semantic_handover/rgb_depth_sync_slop", 0.08))
        self.rgb_depth_sync_queue_size = int(self._get_param(
            "~rgb_depth_sync_queue_size", "/semantic_handover/rgb_depth_sync_queue_size", 5))
        ...
        self.zone_center = (self.zone_min + self.zone_max) / 2.0
        self.zone_half_extents = (self.zone_max - self.zone_min) / 2.0
        self.handover_zone_yaw_deg = float(self._get_param(
            "~handover_zone_yaw_deg", "/semantic_handover/handover_zone_yaw_deg", 0.0))
        self._update_zone_rotation()
        ...
        self.safety_marker_enabled = bool(self._get_param(
            "~safety_marker_enabled", "/semantic_handover/safety_marker_enabled", True))
        self.safety_marker_alpha = float(self._get_param(
            "~safety_marker_alpha", "/semantic_handover/safety_marker_alpha", 0.35))
        self.handover_keepout_radius = float(self._get_param(
            "~handover_keepout_radius", "/semantic_handover/handover_keepout_radius", 0.15))
        ...
        self.rgbd_sync = message_filters.ApproximateTimeSynchronizer(
            [self.color_sub, self.depth_sub],
            queue_size=max(2, self.rgb_depth_sync_queue_size),
            slop=max(0.0, self.rgb_depth_sync_slop),
            allow_headerless=False,
        )
        self.rgbd_sync.registerCallback(self._rgbd_cb)
        ...
        rospy.loginfo(
            "[handover] 啟動完成，發布 hand_state: %s, markers: %s，交接區 min=%s max=%s，RGB-D sync slop=%.3fs queue=%d",
            self.hand_state_topic,
            self.marker_topic,
            self.zone_min.tolist(),
            self.zone_max.tolist(),
            self.rgb_depth_sync_slop,
            self.rgb_depth_sync_queue_size,
        )
    ...
    def _update_zone_rotation(self):
        yaw = math.radians(float(self.handover_zone_yaw_deg))
        c = math.cos(yaw)
        s = math.sin(yaw)
        self.zone_rot = np.array([
            [c, -s, 0.0],
            [s, c, 0.0],
            [0.0, 0.0, 1.0],
        ], dtype=float)
        self.zone_rot_inv = self.zone_rot.T

    def _is_point_in_handover_zone(self, point_base):
        local = self.zone_rot_inv.dot(np.array(point_base, dtype=float) - self.zone_center)
        return bool(np.all(np.abs(local) <= self.zone_half_extents + 1e-9))
    ...
    def _reload_handover_config_if_needed(self, force=False):
        ...
        zone_yaw_deg = float(cfg.get("handover_zone_yaw_deg", self.handover_zone_yaw_deg))
        ...
        safety_marker_enabled = bool(cfg.get("safety_marker_enabled", self.safety_marker_enabled))
        safety_marker_alpha = float(cfg.get("safety_marker_alpha", self.safety_marker_alpha))
        handover_keepout_radius = float(cfg.get("handover_keepout_radius", self.handover_keepout_radius))
        ...
        self.zone_center = (self.zone_min + self.zone_max) / 2.0
        self.zone_half_extents = (self.zone_max - self.zone_min) / 2.0
        self.handover_zone_yaw_deg = zone_yaw_deg
        self._update_zone_rotation()
        self.handover_zone_visible = zone_visible
        self.handover_zone_color_rgba = zone_color_rgba
        self.safety_marker_enabled = safety_marker_enabled
        self.safety_marker_alpha = float(np.clip(safety_marker_alpha, 0.0, 1.0))
        self.handover_keepout_radius = handover_keepout_radius
        ...
        rospy.loginfo(
            "[handover] 已重載 handover config: zone center=%s half=%s yaw=%.1fdeg visible=%s color=%s safety=%s alpha=%.2f keepout=%.3f swap_handedness=%s signs=(R:%s,L:%s) debounce=%d ema=%.2f",
            self.zone_center.tolist(),
            self.zone_half_extents.tolist(),
            self.handover_zone_yaw_deg,
            self.handover_zone_visible,
            self.handover_zone_color_rgba.tolist(),
            self.safety_marker_enabled,
            self.safety_marker_alpha,
    ...
    def _decode_color_image(self, msg: Image):
        try:
    ...
    def _rgbd_cb(self, color_msg: Image, depth_msg: Image):
        stamp_delta = abs((color_msg.header.stamp - depth_msg.header.stamp).to_sec())
        if stamp_delta > self.rgb_depth_sync_slop + 1e-6:
            rospy.logwarn_throttle(
                5.0,
                "[handover] RGB-D 同步差 %.3fs 超過 slop %.3fs，略過此幀",
                stamp_delta,
                self.rgb_depth_sync_slop,
            )
            return

        rgb = self._decode_color_image(color_msg)
        depth = self._decode_depth_image(depth_msg)
        if rgb is None or depth is None:
            return

        self.latest_color_rgb = rgb
        self.latest_color_stamp = color_msg.header.stamp
        if color_msg.header.frame_id:
            self.latest_color_frame = color_msg.header.frame_id
        self.latest_depth = depth
        self.latest_depth_stamp = depth_msg.header.stamp
        if depth_msg.header.frame_id:
            self.latest_depth_frame = depth_msg.header.frame_id
        self.latest_rgb_depth_dt = stamp_delta
    ...
    def _build_hand_state(self):
        ...
        depth_frame = self.latest_depth_frame or self.latest_color_frame
        ...
        state = {
            ...
            "rgb_depth_dt": getattr(self, "latest_rgb_depth_dt", None),
        }
        ...
        point_base = self._transform_point_to_base(point_cam, stamp, depth_frame)
        ...
        landmark_points_base = self._compute_landmark_points_in_base(
            hand_landmarks, depth, stamp, depth_frame)
        raw_handedness = (
        ...
        in_zone = self._is_point_in_handover_zone(point_base)
        if in_zone:
    ...
    def _transform_point_to_base(self, point_cam_xyz, stamp, source_frame=None):
        point_msg = PointStamped()
        point_msg.header.frame_id = source_frame or self.latest_depth_frame or self.latest_color_frame
    ...
    def _compute_landmark_points_in_base(self, hand_landmarks, depth, stamp, depth_frame):
        h, w = depth.shape[:2]
        points = []
    ...
    def _build_base_markers(self, state, landmark_points_base=None):
        ...
        if isinstance(palm_center, (list, tuple)) and len(palm_center) == 3:
            ...
            if self.safety_marker_enabled:
                markers.append(self._make_hand_keepout_marker(
                    stamp,
                    palm_center,
                    self.handover_keepout_radius,
                    10,
                    "handover_keepout",
                    [1.0, 0.25, 0.1, self.safety_marker_alpha]))
    ...
    def _make_zone_marker(self, stamp):
        ...
        marker.pose.position.x = float(self.zone_center[0])
        marker.pose.position.y = float(self.zone_center[1])
        marker.pose.position.z = float(self.zone_center[2])
        marker.pose.orientation.z = math.sin(math.radians(self.handover_zone_yaw_deg) * 0.5)
        marker.pose.orientation.w = math.cos(math.radians(self.handover_zone_yaw_deg) * 0.5)
        size = self.zone_half_extents * 2.0
    ...
    def _make_hand_keepout_marker(self, stamp, palm_center, radius, marker_id, ns, color_rgba):
        marker = self._marker_template(stamp, marker_id, ns, Marker.SPHERE)
